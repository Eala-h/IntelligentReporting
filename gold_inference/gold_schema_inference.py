...
OUTPUT_FILE = f"./pred/pred_{args.ver}_gold_{args.input}.jsonl"
...
def generate_prompt(q):
    ...
    schema = clean_schema(q.get("Schema"))
...
if args.ver != 'base':
    model = PeftModel.from_pretrained(model, FT_MODEL) # Remove if base model only
