# Retriever Ablation: w/o empty tables (tables with no data) 
# All other parts (including: descriptions, stemmed descriptions and exact matching) are retained
...
with open('../data_files/tables.json', 'r', encoding='utf-8') as f, open('../data_files/empty_tables.json', 'r') as fl:
    tables = json.load(f)
...
tables_filtered = [t for t in tables if t['table'].lower() not in empty_tables]
table_names = [t['table'] for t in tables]  # all tables
passage_table_names = [t['table'] for t in tables_filtered]  # only non-empty

passages = [create_passage(t) for t in tables_filtered] # Mainly for BM25 corpus since the FAISS index is already done
...
index = faiss.read_index('faiss_index/schema_subset_index.faiss')
...
def retrieve_faiss(nl_query, top_k=5):
    ...
    for score, i in zip(scores[0], indices[0]):
        if score > 0:
            result.append({"table": passage_table_names[i], "score": f'{float(score):.4f}'})
    return result
...
def exact_name_match(nl_query):
    ...
    matched = []
    for table in passage_table_names:
...
def retrieve_hybrid(nl_query, top_k):
    ...
    return [{"table": t, "score": f"{scores[t]:.4f}"} for t in merged][:top_k]
...
result_log.append({'Result of': f'Version w/o empty tables (with desc + stemming + exact match) on {work_set} with top_k = {k} cap',
